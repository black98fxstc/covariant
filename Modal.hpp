...
private:
    ...
    bool &contiguous(const Coordinate &i, const Coordinate &j) noexcept
    {
...
public:
    unsigned int findClusters(const float *density, int pass, const SelectionState &selections) noexcept;
...
unsigned int ModalClustering::findClusters(const float *density, int pass, const SelectionState &selections) noexcept
{
    clusters = 0;
    maxima[0] = 0;
    ...
    for (pv = vertex; pv < vertex + i; pv++)
    {
        ...
        if (clusters > 12) // no need to waste any more time
            return clusters;
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                case edge:
                    ...
                    case 1:
                        ...
                        bounds.addSegment(ColoredRight, pv->i, pv->j, right, left, weight * std::numbers::sqrt2);
                        break;
                    case 2:
                    ...
                    case 3:
                        weight = center_weight + density[pv->i + 1 + (N + 1) * pv->j - (N + 1)];
                        bounds.addSegment(ColoredLeft, pv->i, pv->j - 1, left, right, weight * std::numbers::sqrt2);
                        break;
                    default:
                ...
                case corner:
                    ...
                    case 2:
                        ...
                        bounds.addSegment(ColoredLeft, pv->i, pv->j, right, 0, weight * std::numbers::sqrt2);
                        break;

                    case 4:
                        weight = density[pv->i + (N + 1) * pv->j - (N + 1)] + density[pv->i + 1 + (N + 1) * pv->j];
                        bounds.addSegment(ColoredRight, pv->i, pv->j - 1, right, 0, weight * std::numbers::sqrt2);
                        break;
