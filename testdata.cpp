...
public:
    ...
    int parse_args(int argc, char *argv[])
    {
        ...
        options.add_options()("f,file", "File name for generated test data", cxxopts::value<std::string>()->default_value("test_data"))("variables", "List of variables", cxxopts::value<std::vector<std::string>>())("populations", "List of populations", cxxopts::value<std::vector<std::string>>())("d,dimension", "Dimension of the events", cxxopts::value<unsigned>()->default_value("2"))("n,normal", "Normal distributions", cxxopts::value<unsigned>()->default_value("3"))("exponential", "Exponential distributions", cxxopts::value<unsigned>()->default_value("0"))("snake", "Snake distributions", cxxopts::value<unsigned>()->default_value("0"))("e,events", "Number of events to generate", cxxopts::value<size_t>()->default_value("10000"))("convert", "Convert existing file between ASCII and binary", cxxopts::value<bool>()->default_value("false"))("a,ascii", "Use ASCII format for data files", cxxopts::value<bool>()->default_value("false")->implicit_value("true"))("h,help", "Print usage");
    ...
    int run()
    {
        TestData test_data(params.dimension);
        if (params.convert)
        {
        ...
        else
        {
            ...
            TestData::RandomSample test_sample(params.dimension);
            for (unsigned i = 0; i < params.normal; i++)
                test_sample.subpopulation(new TestData::Normal(params.dimension));
            for (unsigned i = 0; i < params.snake; i++)
                test_sample.subpopulation(new TestData::Snake(params.dimension));
            for (unsigned i = 0; i < params.exponential; i++)
                test_sample.subpopulation(new TestData::Exponential(params.dimension));
